...
interface ItemTextContentViewProps {
    item: Item;
...
const ItemTextContentView: React.FC<ItemTextContentViewProps> = ({ item, isAdminEditable, onEditItemData, onOpenTextReader }) => {
    ...
    const textEntries = item.textContent || [];
    ...
    const onSaveEntry = (index: number, newContent: string) => {
        ...
        onEditItemData(item.existedId, 'textContent', newEntries);
    };
    ...
    const onDeleteOldest = (count: number) => {
        if (!item.existedId) return;
        const newEntries = textEntries.slice(0, Math.max(0, textEntries.length - count));
        onEditItemData(item.existedId, 'textContent', newEntries);
    };

    const onClearAll = () => {
        if (!item.existedId) return;
        onEditItemData(item.existedId, 'textContent', []);
    };
    
    const onAddEntry = (content: string) => {
        if (!item.existedId) return;
        const newEntries = [content, ...textEntries];
        onEditItemData(item.existedId, 'textContent', newEntries);
    };
        ...
        <>
            <div className={`space-y-4 ${FONT_SIZE_CLASSES[fontSizeIndex]}`}>
                ...
                {textEntries.map((entry, index) => {
                    ...
                    const entryTitle = t('Entry #{count}', { count: textEntries.length - index });
                    return (
            ...
            {isAdminEditable && textEntries.length > 0 && (
                <footer className="mt-6 pt-4 border-t border-gray-700/60 flex flex-col sm:flex-row justify-end items-center gap-4 flex-wrap">
                     <div className="flex items-center gap-2">
                        <input type="number" value={deleteCount} onChange={(e) => setDeleteCount(e.target.value)} min="1" max={textEntries.length} className="w-20 bg-gray-700/50 border border-gray-600 rounded-md py-1 px-2 text-sm text-center text-white" aria-label={t("Number of entries to delete")} />
            ...
            <ConfirmationModal isOpen={isDeleteOldestConfirmOpen} onClose={() => setIsDeleteOldestConfirmOpen(false)} onConfirm={handleDeleteOldestConfirm} title={t("Delete Oldest Entries")}><p>{t("Are you sure you want to delete the {count} oldest text entries for {name}?", { count: deleteCount, name: item.name })}</p></ConfirmationModal>
            <ConfirmationModal isOpen={isClearAllConfirmOpen} onClose={() => setIsClearAllConfirmOpen(false)} onConfirm={handleClearAllConfirm} title={t("Clear All Entries")}><p>{t("Are you sure you want to delete all text content for this item? This cannot be undone.")}</p></ConfirmationModal>
        </>
    );
};

export default ItemTextContentView;
