...
import { BookOpenIcon, ArrowPathIcon, PencilSquareIcon } from '@heroicons/react/24/solid';
  ...
  return (
    <div className="space-y-6">
      {history.map((msg, index) => (
        <div key={index} className={`flex items-center group ${msg.sender === 'player' ? 'justify-end' : 'justify-start'}`}>
          {/* For player messages, button is on the left */}
          {msg.sender === 'player' && allowHistoryManipulation && (
            <div className="flex items-center opacity-40 group-hover:opacity-100 transition-opacity mr-2 flex-shrink-0 pointer-events-auto space-x-1">
              <button 
          ...
          {/* For GM/System messages, button is on the right */}
          {msg.sender !== 'player' && allowHistoryManipulation && (
             <div className="flex items-center opacity-40 group-hover:opacity-100 transition-opacity ml-2 flex-shrink-0 pointer-events-auto space-x-1">
              <button 
                onClick={() => onShowEditModal(index, msg)} 
                className="p-1 rounded-full text-gray-400 hover:bg-gray-700 hover:text-white"
                aria-label={t("Edit")}
